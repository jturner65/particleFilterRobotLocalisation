...
public class mySample {
	...
	public static final int numSweepCasts= (int)(180.0f/Lab1Localization.degPerCasts);
...
//		orient=d.z;
	...
	public void setLRFCastsAtOrient(){
		//setLRFCastsAtOrientNew();
		setLRFCastsAtOrientOld();
	}
	...
	public void setLRFCastsAtOrientNew(){//mapPreScanRayCasts
		...
		//idx of where to start in precomputed casts
		int cellIdxOffset = (int)((posOrientStart/p.degPerC2Rad)+numCasts)%numCasts,	  //offset of where to start is first idx below 90 degrees less than passed orientation
			...
			nextCellIdx = ((cellIdxOffset-1)+numCasts)%numCasts;
	...
	//old scan - weirdness with orientation (d.z)
	public void setLRFCastsAtOrientOld(){//mapPreScanRayCasts
		Tuple<Integer,Integer> key = new Tuple<Integer,Integer>((int)d.x, (int)d.y);
//		orient = d.z;
		float[] cellCastAra = m.mapPreScanRayCasts.get(key);
		if(cellCastAra == null){
	...
	public void drawPreCompRCsbeams(){
		p.pushMatrix();p.pushStyle();
		//d.z is sample orientation
		p.translate(d.x,d.y,0);

		//start sweep - scale to display ccw instead of cw 
		//p.scale(1.0f,-1.0f,1.0f);
		p.rotate(d.z, 0,0,p.rotZ);
		p.rotate(-PConstants.PI/2.0f,0,0,p.rotZ);
		//p.translate(2.5f, 0,0);						//move to where laser range finder vals are
		//sweep should be blue(right) to red(left)
		p.drawCastAra(scanDist, 0, p.degPerC2Rad, 1);	
		p.popStyle();p.popMatrix();		
	}//drawPreCompRCsbeams
